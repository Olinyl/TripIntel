...
function normalizeTime(value: unknown): string | undefined {
  const time = toOptionalString(value);
  if (!time) return undefined;
  return ISO_TIME_REGEX.test(time) ? time : undefined;
}
...
function normalizeTripIntent(payload: UnknownRecord): TripIntent {
  ...
  const maxConnectionsRaw = toOptionalNonNegativeInt(payload.maxConnections);
  const maxConnections = maxConnectionsRaw === undefined ? undefined : Math.min(maxConnectionsRaw, 3);
  const departureTimeStart = normalizeTime(payload.departureTimeStart);
  const departureTimeEnd = normalizeTime(payload.departureTimeEnd);
...
function createPrompt(query: string): string {
  return [
    ...
    "  maxConnections?: number; // integer >= 0",
    "  departureTimeStart?: string; // HH:MM (24h)",
    "  departureTimeEnd?: string; // HH:MM (24h)",
    "}",
    "",
    "Rules:",
    ...
    "- If query requests morning/afternoon/evening/night departures, set departureTimeStart/departureTimeEnd.",
    "- Set maxConnections to 0 for direct flights, 1 for one-stop language, etc.",
    "",
    `User query: ${query}`,
  ].join("\n");
}
