...
import {
  getOfferBestValueScore,
  getOfferFareBrand,
  groupAndSortOffersByBestValue,
} from "@/lib/services/offer-management";

type UnknownRecord = Record<string, unknown>;
const DEFAULT_SUPPLIER_TIMEOUT_MS = 10000;
...
function getDuffelClient(): Duffel {
  ...
  if (!token) {
    throw new Error("DUFFEL_API_TOKEN (or DUFFEL_ACCESS_TOKEN) is not configured.");
  }
...
function sanitizeTimeWindow(timeWindow?: TripIntent["departureTimeWindow"]):
  | { from: string; to: string }
  | null {
  if (!timeWindow?.start || !timeWindow?.end) return null;
  const validTimeRegex = /^([01]\d|2[0-3]):[0-5]\d$/;
  if (!validTimeRegex.test(timeWindow.start) || !validTimeRegex.test(timeWindow.end)) return null;
  return {
...
function mapOfferToFlightOffer(offerValue: unknown, intent: TripIntent): FlightOffer {
  ...
  const secondSlice = asRecord(slices[1]);
  ...
  const secondSliceSegments = Array.isArray(secondSlice.segments) ? secondSlice.segments : [];
  const secondSliceFirstSegment = asRecord(secondSliceSegments[0]);
  const secondSliceLastSegment = asRecord(secondSliceSegments[secondSliceSegments.length - 1]);
  const secondSliceOrigin = asString(asRecord(secondSlice.origin).iata_code);
  const secondSliceDestination = asString(asRecord(secondSlice.destination).iata_code);
  ...
  return {
    ...
    metadata: {
      raw: offer,
      fareBrand: getOfferFareBrand(offer),
      bestValueScore: getOfferBestValueScore(offer),
      trip_type: slices.length > 1 ? "round_trip" : "one_way",
      return_leg:
        slices.length > 1
          ? {
              origin: secondSliceOrigin,
              destination: secondSliceDestination,
              departure_time: asString(secondSliceFirstSegment.departing_at),
              arrival_time: asString(secondSliceLastSegment.arriving_at),
            }
          : undefined,
    },
  };
...
export async function searchDuffelFlights(intent: TripIntent): Promise<FlightOffer[]> {
  ...
  const slicesPayload = [
    {
      ...
      departure_time: departureTimeWindow,
      arrival_time: null,
    },
    ...(intent.returnDate
      ? [
  ...
  const groupedAndSortedOffers = groupAndSortOffersByBestValue(offers);

  return groupedAndSortedOffers.map((offerValue) => mapOfferToFlightOffer(offerValue, intent));
}
