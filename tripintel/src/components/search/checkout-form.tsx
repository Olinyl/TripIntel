...
function createExternalSearchUrl(params: {
  ...
  if (params.bookingUrl) return params.bookingUrl;
  const isoDate = params.departureIso ? new Date(params.departureIso).toISOString().slice(0, 10) : "";
  const query = `Flights to ${params.destination} from ${params.origin}${isoDate ? ` on ${isoDate}` : ""}`;
...
export function CheckoutForm({ offerId, offer, seatMaps, userEmail, returnTo }: CheckoutFormProps) {
  ...
  const firstSegment = Array.isArray(firstSlice.segments) && firstSlice.segments[0]
    ...
    : {};
  const bookingUrl = asString(offer.booking_url);
    ...
    <div className="min-h-screen bg-black px-4 py-10 text-zinc-50">
      <div className="mx-auto max-w-3xl space-y-6">
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-500">Complete passenger details to create a live Duffel order in test mode.</p>
          <Link
            href={returnTo}
        ...
        <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-4">
          <p className="text-sm text-zinc-300">{origin} to {destination}</p>
          <p className="mt-1 text-xs text-zinc-500">Departure: {departureText}</p>
          <p className="mt-2 text-lg font-semibold">{currency} {amount}</p>
          <p className="mt-1 text-[11px] text-zinc-500">Offer ID: {offerId}</p>
          <a
            href={externalSearchUrl}
            target="_blank"
            rel="noreferrer"
            className="mt-3 inline-block rounded-md border border-zinc-700 px-2.5 py-1 text-[11px] text-zinc-300 transition-colors hover:border-indigo-500/60 hover:text-indigo-300"
          >
